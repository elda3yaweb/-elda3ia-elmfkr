...
import {
  ...
  defaultChatSessions, defaultArticleFolders, defaultArticles, GUEST_EMAIL, GUEST_CODE,
} from './defaults';
...
interface AppState {
  ...
  registerGoogle: (name: string, email: string, photo: string, phone: string, gender: Contestant['gender'], competitions: string[]) => { ok: boolean; msg: string };
  logout: () => void;
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      registerGoogle: (name, email, photo, phone, gender, competitions) => {
        ...
        user = {
          ...
          secretCode: Math.random().toString(36).slice(2, 8),
      ...
      addComment: (name, text) => {
        const c: CommentItem = {
          ...
          date: new Date().toLocaleString('ar-EG'), approved: false,
        };
    ...
    }),
    {
      ...
      merge: (persisted: unknown, current: AppState): AppState => {
        ...
        if (!Array.isArray(safeSettings.instructions)) safeSettings.instructions = defaultSettings.instructions;
