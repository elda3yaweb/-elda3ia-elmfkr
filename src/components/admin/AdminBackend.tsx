...
import { getWebAppUrl, setWebAppUrl, testConnection, isLiveMode } from '../../lib/api';
...
export default function AdminBackend() {
  const { settings, updateSettings, syncFromSheets, syncAllFromLive } = useStore();
  ...
  const syncNow = async () => {
    setTesting(true);
    const r = await syncAllFromLive();
    setStatus({ ok: r.ok, msg: r.msg });
    setTesting(false);
  };
  ...
  return (
    <div>
      ...
      <div className="mb-6 rounded-2xl border border-line p-5">
        <h3 className="mb-3 flex items-center gap-2 font-display text-lg font-bold"><FileCode size={20} className="text-primary" /> بنية الشيتات (تابات منفصلة)</h3>
        <div className="space-y-3 text-sm">
          <div className="rounded-xl bg-[color-mix(in_srgb,var(--c-primary)_8%,transparent)] p-3">
            <p className="mb-1 font-bold text-primary">📊 شيت المسابقات — تابات منفصلة:</p>
            <ul className="list-inside list-disc space-y-1 text-muted">
              <li><b>بيانات المتسابقين</b> — كل بيانات الحسابات</li>
              <li><b>إعدادات الاختبارات</b> — مُجمّع كل بنك (الاسم، عدد الأسئلة، الوقت، المسابقات المتاحة، نسبة النجاح...)</li>
              <li><b>تاب لكل بنك</b> — اسم التاب = اسم الاختبار بالظبط من تاب إعدادات الاختبارات، وبداخله الأسئلة:<br/>
                <span dir="rtl" className="text-xs">السؤال · رابط صورة السؤال · الاختيار 1..8 · الإجابة الصحيحة · الشرح</span></li>
              <li><b>النتائج · لوحة الشرف · المذكرات · الفيديوهات</b> — كل واحد في تاب</li>
            </ul>
          </div>
          <div className="rounded-xl bg-[color-mix(in_srgb,var(--c-secondary)_8%,transparent)] p-3">
            <p className="mb-1 font-bold text-secondary">⚙️ شيت الإعدادات — تابات منفصلة:</p>
            <p className="text-muted">الهيدر · الفوتر · شاشة الترحيب · التعليمات · الألوان (ليل/نهار) · العلامة المائية · شريط الإعلان · أيقونات التواصل · الاشتراك · SEO — كل قسم في تابه الخاص.</p>
          </div>
          <div className="flex items-start gap-2 rounded-xl bg-green-500/10 p-3 text-xs text-green-700">
            <Check size={16} className="mt-0.5 shrink-0" />
            المنصة تقرأ كل بنك من تابه المنفصل تلقائياً، وعند استيراد أسئلة من لوحة التحكم يُنشأ التاب باسم البنك تلقائياً إن لم يكن موجوداً.
          </div>
        </div>
      </div>
