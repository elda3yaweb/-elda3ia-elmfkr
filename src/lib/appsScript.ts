...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'importQuestions': out = { ok: true, data: importQuestionsToBank(req.bank, req.rows, req.replace) }; break;
...
function getAll() {
  var result = {};
  // كل صفحات شيت المسابقات (المتسابقين/المسابقات/الاختبارات/بنك الأسئلة...)
  SpreadsheetApp.openById(COMPETITIONS_SHEET_ID).getSheets().forEach(function (sh) {
    result[sh.getName()] = sh.getDataRange().getValues();
  });
  // صفحات شيت الإعدادات
  try {
    SpreadsheetApp.openById(SETTINGS_SHEET_ID).getSheets().forEach(function (sh) {
      result['الإعدادات::' + sh.getName()] = sh.getDataRange().getValues();
    });
  } catch (e) {}
  return result;
}
...
// كل بنك في تاب منفصل باسمه — يُنشأ التاب تلقائياً إن لم يكن موجوداً
function importQuestionsToBank(bankName, rows, replace) {
  var ss = SpreadsheetApp.openById(COMPETITIONS_SHEET_ID);
  var sh = ss.getSheetByName(bankName);
  if (!sh) {
    sh = ss.insertSheet(bankName);
    sh.appendRow(['السؤال','رابط صورة السؤال','الاختيار 1','الاختيار 2','الاختيار 3','الاختيار 4','الاختيار 5','الاختيار 6','الاختيار 7','الاختيار 8','الإجابة الصحيحة','الشرح']);
  } else if (replace) {
...
// يكتب الإعدادات كمفتاح/قيمة، ويُحدّث الموجود بدل تكرار الصفوف
function updateSettings(obj) {
  var sh = SpreadsheetApp.openById(SETTINGS_SHEET_ID).getSheets()[0];
  var data = sh.getDataRange().getValues();
  var index = {};
  for (var i = 0; i < data.length; i++) { index[String(data[i][0])] = i + 1; }
  Object.keys(obj).forEach(function (k) {
    var val = typeof obj[k] === 'object' ? JSON.stringify(obj[k]) : obj[k];
    if (index[k]) {
      sh.getRange(index[k], 2).setValue(val);
    } else {
      sh.appendRow([k, val]);
    }
  });
