...
import { googleSignIn, makeDemoGoogleProfile } from '../lib/google';
import Layout from '../components/Layout';

type Mode = 'login' | 'register' | 'forgot' | 'guest';

export default function Auth() {
  const { login, register, loginGuest, registerGoogle, googleProfileExists, resetPassword, competitions, settings } = useStore();
  ...
  const handleGoogle = async () => {
    setMsg(null);
    let profile: { name: string; email: string; picture: string; gender: Gender };

    if (settings.googleClientId) {
      // REAL Google Sign-In: pulls name/email/picture automatically
      try {
        const p = await googleSignIn(settings.googleClientId);
        profile = { name: p.name, email: p.email, picture: p.picture, gender: 'ذكر' };
      } catch (e: any) {
        setMsg({ ok: false, text: e?.message || 'تعذّر تسجيل الدخول بجوجل' });
        return;
      }
    } else {
      // demo mode: realistic auto-pulled account
      profile = makeDemoGoogleProfile();
    }

    // returning user → just log them in
    if (googleProfileExists(profile.email)) {
      const r = registerGoogle(profile.name, profile.email, profile.picture, '', profile.gender, []);
      setMsg({ ok: r.ok, text: r.msg });
      if (r.ok) setTimeout(() => navigate('/home'), 700);
      return;
    }

    // new user → go complete the rest of the data (whatsapp + competition + secret code)
    navigate('/complete-signup', { state: { profile } });
  };
