...
import { Section, NumField, AreaField, Toggle, SaveBar, TextField } from './ui';
...
export default function AdminDisplay() {
  ...
  return (
    <div>
      ...
      <Section title="قسم التعليقات">
        <Toggle label="إظهار التعليقات" value={s.commentsEnabled} onChange={(v) => set({ commentsEnabled: v })} />
        <TextField label="اسم قسم التعليقات (يمكن تغييره لأي اسم)" value={s.commentsTitle} onChange={(v) => set({ commentsTitle: v })} />
      </Section>
